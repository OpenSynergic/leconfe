        ...
        <div class="card-body space-y-2 bg-gray-50 border">
            <div class="cf-current space-y-1 p-4">
                <div class="flex justify-between">
                    <div class="card-title font-normal">{{ $activeConference->name }}</div>
                </div>
                ...
                @if ($activeConference->hasMeta('date_held'))
                    <div class="pb-2">
                        <div class="inline-flex items-center space-x-2">
                            ...
                            <span class="font-normal text-xs">{{ $activeConference->getMeta('date_held') }}</span>
                        </div>
                    </div>
                @endif
                    ...
                    @if ($activeConference->hasMedia('thumbnail'))
                        <div class="cf-thumbnail w-[1240px] sm:max-w-[12rem]">
                            <img class="h-full w-full rounded object-cover sm:object-left-top"
                                src="{{ $activeConference->getFirstMediaUrl('thumbnail', 'thumb') }}" alt="" />
                        </div>
                    @endif
                    <div class="flex flex-col gap-2">
                        @if ($activeConference->hasMeta('description'))
                            <div class="cf-description h-full w-full">
                                <p class="text-[.65rem]">{!! $activeConference->getMeta('description') !!}</p>
                ...
                <div class="max-w-7x1 container mx-auto py-5">
                    <div class="mb-4 flex w-full flex-wrap p-4">
                        <div class="mb-6 w-full lg:mb-0">
                            <h1 class="card-title font-normal">Upcoming Conferences</h1>
                        </div>
                    </div>
