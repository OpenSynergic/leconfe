    ...
    <div class="relative">
        <ul class="navbar-items flex items-center justify-center flex-1 p-1 space-x-1 list-none group">
            @foreach ($items as $key => $item)
                ...
                @else
                    <x-website::navigation.dropdown.trigger :key="$key" :label="$item['label']" />
                @endif
            @endforeach
        </ul>
    </div>
    <div x-navigation:dropdown
        class="navbar-dropdown absolute -top-2 duration-200 ease-out -translate-x-1/2 translate-y-11 transition-all"
