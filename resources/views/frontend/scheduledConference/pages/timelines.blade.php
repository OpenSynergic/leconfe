            ...
            @foreach ($timelines as $timeline)
                <li class="mb-10 ms-4 last:mb-0">
                    ...
                    <time class="mb-1 text-sm font-normal leading-none text-gray-400 dark:text-gray-500" datetime="{{ $timeline->date?->toIso8601String() }}">
                        {{ $timeline->formattedFullDate($currentScheduledConferenceTimezone ?? null) }} @if(!empty($currentScheduledConferenceTimezone))({{ $currentScheduledConferenceTimezone }})@endif
                    </time>
