                ...
                <div @class([
                    ...
                    @if ($activeConference->hasMeta('date_held'))
                        <div class="inline-flex items-center space-x-2">
                            ...
                            <time
                                class="text-xs text-secondary">{{ date(setting('format.date'), strtotime($activeConference->getMeta('date_held'))) }}</time>
                        </div>
                    @endif
                    ...
                    @foreach ($upcomingConferences as $conference)
                        <div class="cf-upcoming-conference h-full">
                            <div class="rounded border bg-white p-2">
                                ...
                                @if ($conference->hasMeta('date_held'))
                                    <div class="cf-upcomiong-enrollment text-xs">Enrollment start on
                                        {{ date(setting('format.date'), strtotime($conference->getMeta('date_held'))) }}
                                    </div>
                                @endif
